...
public class Parser {
	...
	public void testComplexLeft() {
		FormationTree tree = compiler.compile("(¬pv(s^t))^q");
		assertEquals("(¬pv(s^t))^q: ", tree.toTreeString(), "0-0: ^ (0-1: v (0-2: ¬ (0-3: p), 1-2: ^ (2-3: s, 3-3: t)), 1-1: q)");
	}

	@Test
	public void testComplexRight() {
		FormationTree tree = compiler.compile("q^(¬pv(s^t))");
		assertEquals("q^(¬pv(s^t)): ", tree.toTreeString(), "0-0: ^ (0-1: q, 1-1: v (2-2: ¬ (4-3: p), 3-2: ^ (6-3: s, 7-3: t)))");
	}
